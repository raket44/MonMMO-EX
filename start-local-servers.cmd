...
if exist "server.game\build\install\server.game\lib" (
  start "MonMMO Game Server" /min /d "%~dp0server.game" cmd /c ""%JAVA_HOME%\bin\java.exe" -cp "build\install\server.game\lib\*" de.fiereu.openmmo.server.game.MainKt > "%~dp0logs\server-game.log" 2> "%~dp0logs\server-game.err.log""
) else (
