 ...
 * and updated by the character delta's 0x100 field), shows 32 minus that as pp, and refuses the
 * use, offering Leppa Berries, once the spent byte reaches the move's cost. When it does use it,
...
constructor(
  ...
  /** The pp the ocarina has spent, for the character record's byte. */
  fun ppSpent(stored: StoredCharacter): Int = (stored.storyVars[SPENT_KEY] ?: 0).coerceIn(0, MAX_PP)
  ...
  suspend fun use(ctx: SessionContext, state: PlayerState, charId: Long, itemId: Int) {
    ...
    if (spent + USE_COST > MAX_PP) {
      ...
      ctx.send(LocalCharacterDeltaPacket(value256 = spent.toByte()))
      return
    }
    ...
    ctx.send(LocalCharacterDeltaPacket(value256 = now.toByte()))
  ...
  fun refill(ctx: SessionContext, charId: Long) {
    ...
    ctx.send(LocalCharacterDeltaPacket(value256 = 0))
