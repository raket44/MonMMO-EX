...
public class Raw5{
 ...
 public static void main(String[] a)throws Exception{rom=Files.readAllBytes(Paths.get(a[0]));walk(u32(rom,0x40),0,"");for(int k=2;k<a.length;k++){byte[] f=narcFile(a[1],Integer.parseInt(a[k]));StringBuilder sb=new StringBuilder(a[1]+" file "+a[k]+" len "+f.length+": ");for(int i=0;i<Math.min(f.length,64);i++)sb.append(String.format("%02x ",f[i]&0xFF));System.out.println(sb);}}
