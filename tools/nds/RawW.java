...
public class RawW{
 ...
 public static void main(String[]a)throws Exception{
  ...
  int npc=f[4]&0xFF,sec=f[5]&0xFF,wr=f[6]&0xFF,tg=f[7]&0xFF;
  System.out.println("file "+idx+" npc="+npc+" sec="+sec+" warp="+wr+" trig="+tg+" len="+f.length);
  int o=8+npc*20+sec*36;
  for(int i=0;i<wr;i++){int q=o+i*20;StringBuilder sb=new StringBuilder();
   for(int k=0;k<20;k++)sb.append(String.format("%02x ",f[q+k]));
   System.out.println(" w"+i+": "+sb+" | s16s: "+s16(f,q)+","+s16(f,q+2)+","+s16(f,q+4)+","+s16(f,q+6)+","+s16(f,q+8)+","+s16(f,q+10)+","+s16(f,q+12)+","+s16(f,q+14)+","+s16(f,q+16)+","+s16(f,q+18));}
  int t=o+wr*20;
  System.out.println("trailing ("+(f.length-t)+" bytes):");
  for(int i=t;i<f.length;i+=2)System.out.print(s16(f,i)+" ");
  System.out.println();
 }
